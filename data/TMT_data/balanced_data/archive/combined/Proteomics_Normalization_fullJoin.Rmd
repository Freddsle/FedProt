---
title: 'Normalization of Proteomics Data for Centers 1+2+3'
...
PG files merged using only shared PG.

## Functions for reading and merging MQ outputs
```{r include=FALSE}
preprocess_mxout <- function(path, center_metadata, center){
    ...
    pg_intensities <- PG_report %>%
        select(c("Majority.protein.IDs", "Gene.names", rownames(center_metadata)))
...
```{r config}
#meta_file <-  paste0("/home/yuliya/repos/cosybio/FedProt/data/TMT_data/balanced_data/combined/metadata_combined.tsv")
...
final_out_dir <-  paste0("/home/yuliya/repos/cosybio/FedProt/data/TMT_data/balanced_data/combined/normalization_results_full")
...
for(center in names(list_of_outputs)) {
    ...
    pg_intensities <- pg_intensities[, c("Majority.protein.IDs", "Gene.names", rownames(center_metadata))]
    colnames(pg_intensities) <- c("Majority.protein.IDs", "Gene.names", center_metadata$Quantitative.column.name)
    
    if(!(nrow(combined_pg_intensities) == 0)) {
        combined_pg_intensities <- full_join(combined_pg_intensities, pg_intensities, by = c("Majority.protein.IDs", "Gene.names"))
    } else {
        combined_pg_intensities <- pg_intensities
    }
}

combined_pg_intensities <- combined_pg_intensities[, c("Majority.protein.IDs", "Gene.names", metadata$Quantitative.column.name)]
...
md <- md[, .SD, .SDcols = c("Column", "TMT", "Batch", "Condition", "Label", "Center")]
...
se <- load_data(data = prot_data, md = md, 
                protein_column = "Majority.protein.IDs", gene_column = "Gene.names", 
...
```{r}
limmaNorm_2 <- function(se, ain = "log2", aout = "limBE", on_raw = FALSE){
  ...
  design <- model.matrix(~0 + metadata(se)$Group)
  dt_batch <- limma::removeBatchEffect(dt, batch = batch_vector, design = design)
...
normalize_se_single <- function(se, methods = NULL, gamma.0 = 0.5, reduce_correlation_by = 1, NormicsVSN_quantile = 0.8, top_x = 50, VSN_quantile = 0.9){
  ...
  norm_functions <- norm_functions <- list(globalMeanNorm, globalMedianNorm, medianNorm, meanNorm, irsNorm,
                                           ...
                                           rlrMANorm, rlrMACycNorm, eigenMSNorm, medianAbsDevNorm, robNorm, tmmNorm, limmaNorm_2, normicsNorm, normicsNorm)
...
p3 <- plot_PCA(se_normalized, ain = normalization_methods, color_by = "Condition", 
               label_by = "No", shape_by = "Center", facet_norm = TRUE, ncol = 2) + scale_color_brewer(palette = "Set1")

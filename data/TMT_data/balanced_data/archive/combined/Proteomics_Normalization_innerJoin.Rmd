...
final_out_dir <-  paste0("/home/yuliya/repos/cosybio/FedProt/data/TMT_data/balanced_data/combined/normalization_results_inner")
...
for(center in names(list_of_outputs)) {
    ...
    if(!(nrow(combined_pg_intensities) == 0)) {
        combined_pg_intensities <- inner_join(combined_pg_intensities, pg_intensities, by = c("Majority.protein.IDs", "Gene.names"))
    } else {
...
normalization_methods <- c("IRS", "limBE", "limBE_on_Median", "IRS_on_Median", "limBE_on_IRS_on_Median", "Median_on_IRS", "limBE_on_Median_on_IRS") 

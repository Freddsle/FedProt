...
preprocess_mxout <- function(path, center_metadata, center){
    ...
    pg_intensities <- PG_report %>%
        select(c("Majority.protein.IDs", "Fasta.headers", "Gene.names", rownames(center_metadata))) %>%
        mutate(Protein.IDs = ifelse(Fasta.headers == "", Majority.protein.IDs, Fasta.headers)) %>%
        select(-c(Majority.protein.IDs, Fasta.headers))
    
    pg_intensities$Protein.IDs <-sapply(pg_intensities$Protein.IDs, function(x) unique(unlist(strsplit(x, ";"))))
    pg_intensities$Protein.IDs <- sapply(pg_intensities$Protein.IDs, function(x) x[x != ""])
    pg_intensities$Protein.IDs <- pg_intensities$Protein.IDs %>%
        sapply(function(x) paste(x, collapse = ";")) %>% as.character()
    pg_intensities <- pg_intensities %>%
        group_by(Protein.IDs, Gene.names) %>%
    ...
    pg_intensities <- pg_intensities %>%
      ...
      ungroup()  %>%
      mutate(Protein.IDs = if_else(duplicate, paste(Protein.IDs, Gene.names, sep = "_"), Protein.IDs)) %>%
      select(-duplicate) %>% as.data.frame()
    ...
    pg_counts <- PG_report %>%
        ...
        select(-c(Majority.protein.IDs, Fasta.headers)) %>%
        group_by(Protein.IDs) %>%
        summarise(Razor...unique.peptides = min(Razor...unique.peptides)) %>% as.data.frame()
...
final_out_dir <-  paste0("/home/yuliya/repos/cosybio/FedProt/data/TMT_data/balanced_data/combined/normalization_results_magic")
...
normalization_methods <- c("Median") 

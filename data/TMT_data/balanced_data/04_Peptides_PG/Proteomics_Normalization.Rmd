...
#prot_file <- "/home/yuliya/repos/cosybio/FedProt/data/TMT_data/balanced_data/04_Peptides_PG/merged_intensities.tsv"
meta_file <-  "/home/yuliya/repos/cosybio/FedProt/data/TMT_data/raw_MaxQuant_reports/Metadata_CosyBio.tsv"

save_plots <- TRUE
# Define the final output directory or set NULL if results should note be saved
final_out_dir <-  paste0("/home/yuliya/repos/cosybio/FedProt/data/TMT_data/balanced_data/04_Peptides_PG/normalization_results")
...
#prot_data$Gene.names <- c(1:dim(prot_data)[1])
...
intensities <- lapply(centers, function(center) {
  read_tsv(file.path(path_to_data, paste0(center, "/intensities_filtered.tsv")), show_col_types = FALSE)
})

# print number of rows in each data frame
print(paste("Number of rows in each data frame: ", sapply(intensities, nrow)))

# merge intensities using major_features column
intensities <- Reduce(function(x, y) {
  merge(x, y, by = "major_features")
}, intensities)
intensities <- intensities %>% mutate_all(~replace(., . == 0, NA)) %>%
  column_to_rownames("major_features")
...
prot_data <- intensities[,metadata$Quantitative.column.name] %>% as.data.frame() %>% rownames_to_column("PG") %>% 
  as.data.table()
```
...
```{r change_meta}
metadata <- meta_data %>% as.data.frame()
metadata$Column <- metadata$'Quantitative column name'

metadata <- metadata %>%
            mutate(Column = gsub(" Pool", ".P_", gsub("Reporter intensity corrected ", "RIC_", Column)))


# remove the outlier
metadata <- metadata %>%
            filter(Column != "RIC_3.P_3")


# rename the columns - put the .P_[1-9]+ after dot before and the rest after (e.g. RIC_1.P_1 -> P_1.RIC_1)
metadata <- metadata %>%
            mutate(Column = gsub("RIC_([0-9]+).P_([0-9]+)", "P_\\2.RIC_\\1", Column))



# Make copy of meta_data
md <- metadata %>% as.data.table()
...
```{r}
prot_data$Genes <- prot_data$PG

se <- load_data(data = prot_data, md = md, 
                protein_column = "PG", gene_column = "Genes", 
                ref_samples = comref_meta_data, 
                batch_column = "Batch", condition_column = "Condition", 
                label_column = "Label")
```
...
normalization_methods <- c("Median", "IRS_on_Median", "limBE_on_IRS_on_Median", "NormicsVSN", "limBE_on_NormicsVSN", "RobNorm")
#, ""NormicsMedin"", "limBE_on_NormicsMedin", "limBE_on_NormicsVSN", "RobNorm") 

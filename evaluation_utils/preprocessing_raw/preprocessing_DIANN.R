...
read_and_preprocc <- function(path, name, use_filter=TRUE) {
    # read data
...
extract_pg_matrix <- function(data, type_output="DIANN") {
  if (type_output == "DIANN"){
  ...
  } else if (type_output == "Spectronaut"){
    pg_matrix <- data %>% 
        select(Protein.Group, Run, PG.Quantity) %>% 
        unique() %>% 
        pivot_wider(names_from = Run, values_from = PG.Quantity) %>% 
        column_to_rownames("Protein.Group")
  }
  pg_matrix <- pg_matrix %>% 
    # transform 0 to NA
...
get_pep_counts_table <- function(data, sample_names=NULL, do_filter=TRUE) {
    if (do_filter){
        data <- data %>%
...
get_pep_counts_spectronaut <- function(data){
    summ.prec.count.table <- data %>%
        select(Protein.Group, PG.Count) %>%
        rename(count = PG.Count) %>%
        unique() %>%
        # for duplicating rows, keep the row with minimum count
        group_by(Protein.Group) %>%
        summarise(count = min(count)) %>%
        as.data.frame(.)

    # if there is zeros in count, add 1 to all counts
    if (any(summ.prec.count.table$count == 0)){
        summ.prec.count.table$count <- summ.prec.count.table$count + 1
    }

    rownames(summ.prec.count.table) <- summ.prec.count.table$Protein.Group
    summ.prec.count.table$Protein.Group <- NULL

    return(list(summ.prec.count.table, NULL))
